...
from musetalk.utils.blending import get_image, get_image_prepare_material, \
    get_image_blending
from musetalk.utils.utils import load_all_model, load_diffusion_model, \
    load_audio_model
...
def inference(render_event, batch_size, latents_out_path, audio_feat_queue,
              audio_out_queue, res_frame_queue,
...
class MuseReal:
    def __init__(self, opt):
        ...
        mp.Process(target=inference, args=(
        self.render_event, self.batch_size, self.latents_out_path,
    ...
    def __loadavatar(self):
        ...
        input_mask_list = sorted(input_mask_list, key=lambda x: int(
            os.path.splitext(os.path.basename(x))[0]))
    ...
    def process_frames(self, quit_event, loop=None, audio_track=None,
                       video_track=None):
        ...
        while not quit_event.is_set():
            try:
                ...
                continue
            if audio_frames[0][1] == 1 and audio_frames[1][
                1] == 1:  # 全为静音数据，只需要取fullimg
                ...
                # t=time.perf_counter()
                combine_frame = get_image_blending(ori_frame, res_frame, bbox,
                                                   mask, mask_crop_box)
